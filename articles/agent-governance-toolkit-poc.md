...
こうした仕組みにより、エージェントは自律的に判断し、複数のツールを連鎖的に呼び出して仕事をこなせるようになりました。
...
人間が都度確認するワークフローならまだしも、自律的に動くエージェントではこうしたリスクが**気づかないうちに**顕在化します。

## Agent Governance Toolkitの適用

Agent Governance Toolkitは、こうした自律的なエージェントに対してミドルウェアとしてガバナンスを差し込めます。12以上のフレームワークに対応しています。
...
ポイントは、**LLMが「使える」と判断しても、ミドルウェアが実行前にブロックする**という点です。LLMの判断とガバナンスの判定は別レイヤーなので、プロンプトインジェクション等でLLMが騙されても、ルールベースのミドルウェアが最後の砦になります。ガバナンスの評価は<0.1msなので、LLM API呼び出し（数百ms〜数秒）と比べてオーバーヘッドは無視できるレベルです。
