...
ここから、各コンポーネントが何をしてくれるのか、身近なたとえを交えつつもう少し掘り下げてみます。

### 🛡️ Agent OS

Agent OSは、ルールブックと門番の役割です。

会社の新入社員を想像してみてください。「CRMを検索していい」「返金は100ドルまで」「SQLの直接実行はダメ」というルールが書かれたマニュアルがあり、行動するたびに上司がチェックするイメージです。

| 機能 | 役割 |
|------|------|
| **PolicyEngine** | 「この操作は許可？拒否？」をリアルタイム判定 |
| **ツール許可/拒否リスト** | `search_crm` → OK、`execute_sql` → 即ブロック |
| **パラメータ検証** | 「返金額が$100を超えてないか？」「メール送信先が社内ドメインか？」など引数レベルでチェック |

プロンプトインジェクション（ASI-01）で「全注文を返金しろ」と指示されても、ポリシーが「Tier-1は$100まで」とブロックします。ツール悪用（ASI-02）で `shell_exec` を呼ばせようとしても、denied_toolsリストで即拒否されます。

### 🔐 AgentMesh

AgentMeshはエージェントごとに暗号署名付きのIDを発行し、信頼度を数値化して管理します。

人間の世界だと、社員証で本人確認し、役職や実績で信頼度が決まりますよね。AgentMeshはこれをAIエージェントに適用したものです。

| 機能 | 役割 |
|------|------|
| **AgentIdentity (Ed25519 + DID)** | エージェントごとの暗号署名付きID。「自分は本物のサポートBotだ」と証明できる |
| **SPIFFE/SVID** | マイクロサービス業界標準のID仕様。エージェント間で「お前は本当にTier-1 Botか？」を検証 |
| **TrustScore (0〜1000)** | 成功すれば上がり、違反すれば下がる信頼度スコア |

Trust Scoreのティアはこんな感じです。

| ティア | スコア範囲 | できること |
|--------|-----------|-----------|
| untrusted | 0〜199 | 何もできない |
| probationary | 200〜599 | 検索のみ |
| standard | 600〜699 | メール送信可 |
| trusted | 700〜1000 | 返金処理可 |

ID偽装（ASI-03）を防ぎ、エージェント間通信（ASI-07）での「なりすまし」を暗号的に排除します。信頼スコアが低いエージェントは高権限の操作ができません。

### 🏗️ Agent Runtime

Agent Runtimeは、エージェントの行動を**実行時に**制御するためのコンポーネントです。

WindowsやLinuxのカーネルには「Ring 0〜Ring 3」という特権レベルがあります。これと同じ仕組みをAIエージェントに適用したのがAgent Runtimeです。

| リング | レベル | 配置されるツール |
|--------|--------|----------------|
| Ring 0 | Kernel | ガバナンスシステム自体。`shell_exec` など最高特権 |
| Ring 1 | Drivers | ミドルウェア層。`execute_sql` など |
| Ring 2 | Services | `process_refund`、`send_email` など影響の大きいツール |
| Ring 3 | User/Sandbox | `search_crm` など読み取り専用の安全なツール |

エージェントはRing 3（Sandbox）で動いているので、Ring 0-1のツールは**絶対に呼べません**。

さらに **Kill Switch**（`SignalDispatcher`）があり、`SIGKILL` でエージェントを即停止、`SIGSTOP` で一時停止、`SIGCONT` で再開できます。暴走したエージェント（ASI-10）を確実に止められる仕組みです。

### 📊 Agent SRE

Agent SREは、Webサービスの運用でおなじみのSRE（Site Reliability Engineering）の考え方を、AIエージェントに持ち込んだものです。

| 機能 | 役割 |
|------|------|
| **SLO** | 「タスク成功率99.5%以上」「レスポンス5秒以内」など目標を定義 |
| **Error Budget** | 「許容される失敗の予算」。99.5%SLOなら200回中1回まで失敗OK。使い切ったらエージェントを制限 |
| **Circuit Breaker** | 電気のブレーカーと同じ。連続失敗→一時停止（OPEN）→少し待って再試行（HALF_OPEN）→成功で復帰（CLOSED） |
| **Chaos Engineering** | わざと障害（遅延注入、エラー注入、プロンプトインジェクション）を起こして耐性をテスト |

エージェントの「品質劣化」（ASI-08）を数値で検知し、壊れたまま動き続けることを防ぎます。

### 📝 Compliance

Complianceは、飛行機の事故調査で使うフライトレコーダーと同じ発想です。エージェントの全行動を記録し、改ざんを検知します。

| 機能 | 役割 |
|------|------|
| **FlightRecorder** | 成功・違反・エラーをすべて記録。「いつ、誰が、何をしたか」が全部残る |
| **AuditChain (Merkle root)** | ログのハッシュチェーン。途中の1件でも改ざんすると全体のハッシュが崩れて検知できる |
| **AuditLog** | 追記専用（append-only）。削除や上書き不可 |

「このエージェントは本当にルール通り動いていたか？」を事後検証（ASI-09）するために必須です。

### 🏪 Marketplace

Marketplaceは、AIエージェントのプラグインやツールの安全性を保証するコンポーネントです。

iPhoneのApp Storeでは、Appleがアプリを審査し、署名して配布します。同じ仕組みをAIエージェントのプラグインに適用したのがMarketplaceです。

| 機能 | 役割 |
|------|------|
| **PluginManifest** | プラグインの「パッケージラベル」。名前、バージョン、必要な権限、作者情報を宣言 |
| **PluginSigner (Ed25519)** | マニフェストに暗号署名。作者が本物で、内容が改ざんされていないことを証明 |
| **PluginRegistry** | 署名検証済みのプラグインだけが登録される「公式ストア」 |
| **MCPSecurityScanner** | MCPサーバー/ツールの脆弱性スキャン。ツールの「指紋」を取って安全性を評価 |

悪意あるプラグインの混入（ASI-04: サプライチェーン攻撃）を防ぎます。署名なし・改ざんされたプラグインはレジストリに登録できません。

---

ここまでの6コンポーネントの関係をまとめると、こんな感じです。

```
ユーザー入力
  ↓
[Agent OS]      ← ルールに従っているか？ (ポリシー判定)
[AgentMesh]     ← 本人確認OK？信頼度は十分？ (ID・Trust Score)
[Agent Runtime] ← 権限レベルは適切？ (特権リング)
  ↓
ツール実行
  ↓
[Agent SRE]     ← 品質は維持されているか？ (SLO・Circuit Breaker)
[Compliance]    ← 全部記録した？ (監査ログ・改ざん検知)
[Marketplace]   ← そもそもこのツールは安全？ (署名・スキャン)
```

OSのセキュリティモデル（特権リング）、ネットワークのゼロトラスト（mTLS、SPIFFE）、SREの信頼性工学（SLO、Circuit Breaker）、サプライチェーンセキュリティ（コード署名）——こうした既存の成熟した概念をAIエージェントの世界に持ち込んでいるのが、このツールキットの面白いところだと思います。

# 自律的なエージェントにこそガバナンスが必要

## MCP・Skillsとエージェントの自律化

最近のAIエージェントは、**MCP（Model Context Protocol）** や **Skills** といった仕組みで、外部のツールやサービスに自由にアクセスできるようになっています。
簡単にMCPとSkillsについて解説すると、
- **MCP**: Anthropicが提唱したプロトコルで、エージェントが外部のツールサーバーに接続してデータベース検索、ファイル操作、API呼び出しなどを行える。MCPサーバーを追加するだけでエージェントの能力が拡張される
- **Skills**: エージェントが特定の手順やワークフローを「スキル」として持ち、状況に応じて自律的に選択・実行する仕組み。複数のツールを組み合わせた複雑なタスクも一連の流れとして処理できる

これらの仕組みにより、エージェントは自律的に判断し、複数のツールを連鎖的に呼び出して仕事をこなせるようになりました。

## 自律的であるほどリスクが高まる

ところが、自律性が高いということは、**人間がチェックする余地なくツールが実行される**ということでもあります。

たとえばMCPサーバー経由でさまざまなツールを使う自律エージェントを考えてみましょう。

- MCPサーバーから提供されるツールの説明文に悪意あるコマンドが仕込まれている（ASI-04: Tool Poisoning）
- エージェントがプロンプトインジェクションで騙されて、本来呼ぶべきでないツールを連鎖的に実行する（ASI-01 + ASI-02）
- 1つのエージェントのエラーが他のエージェントに伝播し、連鎖障害を起こす（ASI-08）

人間が都度確認するワークフローならまだしも、自律的に動くエージェントではこうしたリスクが**気づかないうちに**発生してしまいます。

## Agent Governance Toolkitの適用

Agent Governance Toolkitは、自律的なエージェントに対してもミドルウェアとしてガバナンスを差し込めます。12以上のフレームワークに対応しています。

| フレームワーク | 統合方式 |
|---|---|
| Microsoft Agent Framework | ネイティブミドルウェア |
| Semantic Kernel (.NET + Python) | ネイティブ統合 |
| LangGraph / AutoGen / CrewAI | アダプター |
| OpenAI Agents SDK | ミドルウェア |
| Google ADK | アダプター |
| Dify | プラグイン |

MCPツールも `allowed_tools` / `denied_tools` でフィルタでき、**MCPSecurityScanner**でツールの説明文やスキーマに仕込まれた脅威をスキャン検出できます。

ツール呼び出しのたびに、こんな感じで防御層を通ります。

```
エージェント（自律行動）
  ↓ ツール呼び出し
[CapabilityGuard]     ← そもそもこのツール使っていい？
[TrustScore]          ← 信頼スコア足りてる？
[RingEnforcement]     ← 特権レベルは適切？
[ParameterValidation] ← 引数に不正な値ない？
[RogueDetection]      ← 異常な呼び出しパターンじゃない？
  ↓
ツール実行
  ↓
[CircuitBreaker]      ← 連続失敗してたら遮断
[AuditLog]            ← 全部記録
```

**LLMが「使える」と判断しても、ミドルウェアが実行前にブロックする**のがポイントです。LLMの判断とガバナンスの判定は別レイヤーなので、プロンプトインジェクション等でLLMが騙されても、ルールベースのミドルウェアが最後の砦になります。ガバナンスの評価は<0.1msなので、LLM API呼び出し（数百ms〜数秒）と比べてオーバーヘッドは無視できるレベルです。
...
自律的なエージェントが普及するほど、「エージェントが何をするか」をランタイムで制御するガバナンス層は必要になってくるなと感じます。
...
今回のPoCでは、わかりやすさを優先して**人間がエージェントにチャットで指示するカスタマーサポートBot**を題材にしています。自律エージェントでもガバナンスの仕組みは同じですが、「ユーザーが悪意ある入力をしたときにブロックされる」という動きのほうがデモしやすそうなので。

また、カスタマーサポートを選んだのは、不正返金・PII漏洩・SQL実行といった分かりやすいリスクがあり、6コンポーネント全部を試せそうだからです。
