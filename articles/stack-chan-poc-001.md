---
title: "M5Stack CoreS3 x gpt-realtime-1.5 - （ｽﾀｯｸﾁｬﾝはじめる） -"
emoji: "🐻‍❄️"
type: "tech"
...
1. **音声認識（STT: Speech-to-Text）** -- マイクで拾った音声を文字に変換する。「こんにちは」という音声が「こんにちは」というテキストになる
2. **大規模言語モデル（LLM）** -- テキストを受け取って、テキストで回答を生成する。ChatGPTなどのモデル
3. **音声合成（TTS: Text-to-Speech）** -- テキストの回答を音声に変換する。「こんにちは！元気ですか？」というテキストが音声になる
...
ロボットに「こんにちは」と話しかけてから返事が聞こえるまで、音声データがどう流れるかを追いかけてみましょう。
