...
この「プロンプト × ツール」の概念を、Foundryでは以下のように実現しています
...
Microsoft Foundryは、AIエージェントの基盤として、多様なモデルをサポートしていることがわかります。
AnthropicのClaudeモデルは、ツール呼び出しに強みがあるので、ツールが多く、目的達成のためにツールを使いこなす必要がある場合によく使っています。
...
**微調整 Fine-tuning**は、基盤モデルを独自のデータセットでカスタマイズし、特定のタスクに最適化する機能です。エージェントの「プロンプト」部分をより強化する手段です。

```mermaid
flowchart LR
    subgraph FineTuning["微調整のメリット"]
        Q["高品質な結果"]
        E["多くの例で学習"]
        T["トークン節約"]
        L["低レイテンシ"]
    ...
    Base["基盤モデル"] --> FT["微調整"] --> Custom["カスタムモデル"]
```
