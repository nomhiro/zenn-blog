...
書類の分類器やブラウザ操作の効率化はもちろん、Doomやスーパーマリオ、スネークをJevにプレイさせてみたり、Wikipediaのリンクをたどるレースをさせてみたりと、面白いネタは皆さんがたくさん考えてくださってます。アイデアマンではなくて、あまり面白いネタが思いつかないので、渋めなところの利用用途を考えてみました。

https://typesafe.ai/blog/introducing-system-one-models-and-jev
https://github.com/fhshaik/typesafe-mario
https://github.com/sorrycc/typesafe-snake
...
動作確認用に、話題が近い3種類の文書から合成的にチャンクを作りました（実際の政府文書の引用ではなく、説明用のダミーテキストです）。ノイズは、正解の文の主語を「事業者」に差し替えて言い回しを近づけ、対象読者だけが違うケースを意図的に作っています。

| doc_id | issuer | section | 内容の方向性 |
| --- | --- | --- | --- |
| 行政AI利活用ガイドライン | デジタル庁 | 調達手続 / リスク評価 | 質問に対する **正解** |
| AI事業者ガイドライン | 総務省・経済産業省 | 調達手続 / リスク評価 | 話題も言い回しも近いが **対象読者が違う** ノイズ |
...
score=0.7658  issuer=デジタル庁         行政AI利活用ガイドライン / 調達手続
score=0.7611  issuer=総務省・経済産業省  AI事業者ガイドライン / リスク評価
score=0.6790  issuer=総務省・経済産業省  AI事業者ガイドライン / 調達手続
score=0.2297  issuer=観光庁            観光白書 / 施策
score=0.1403  issuer=観光庁            観光白書 / 概況
```

デジタル庁の2チャンクが上位に来る一方で、対象読者が違う「AI事業者ガイドライン」のチャンクが、正解の2位（0.7658）にほぼ並ぶ0.7611で入ってきました。差はわずか0.005です。閾値を0.70にすると混ざりますし、0.77にすると今度は正解の2位が落ちてしまいます。言い回しがほぼ同じなので、ベクトル類似度だけでは切り分けにくいんですよね。ここがまさに設計段階で想定していた「話題は似ているけど対象が違う」パターンです。

![](/images/jev-cosmosdb-rag-filter-poc/rag-noise-problem.png)
*正解の2位（デジタル庁）とノイズの1位（総務省・経産省）の類似度は、0.77と0.76でほとんど差がない*
...
| デジタル庁 / 調達手続 | 0.92 | 0.74 | 0.69 | ✅ 採用 |
| 総務省・経産省 / リスク評価 | 0.93 | 0.34 | 0.81 | ❌ 除外 |
| 総務省・経産省 / 調達手続 | 0.87 | 0.26 | 0.70 | ❌ 除外 |
| 観光庁 / 施策 | 0.05 | 0.30 | 0.03 | ❌ 除外 |
| 観光庁 / 概況 | 0.02 | 0.23 | 0.02 | ❌ 除外 |

ベクトル検索で正解の2位に並んでいた「AI事業者ガイドライン」のチャンクは、`scope_matches`が0.34/0.26と、しきい値の0.50をしっかり下回りました。「対象読者が違う」ことをJevが検出できています。同じコードを3回実行しても採否は変わらず、`scope_matches`は正解が0.65〜0.74、ノイズが0.25〜0.36の範囲でした。面白いのは、この2件の`is_relevant`（0.93/0.87）と`has_answer_evidence`（0.81/0.70）が高いままなことです。話題は合っていて、回答っぽい記述もあるので、この2つの観点だけだと通してしまいます。対象読者を別の観点として聞いたから弾けました。観光白書の方は`is_relevant`が0.05/0.02とそもそも無関係だと一発で判定されていて、こちらは想定通りの結果でした。
...
Step 2で`scope_matches`が0.34/0.26と低く出て、「対象読者が違う」ことは検出できました。でもよく考えると**なぜ0.34という数字になったのか**、Jevは一切教えてくれません。監査や規制対応が絡む業務に組み込むなら無視できないポイントなので、ここだけ少し掘って調べてみました。
...
- ベクトル検索だけでは「話題は似ているが対象読者が違う」ノイズを弾きにくいが、Jevの`scope_matches`のような観点別判定を挟むことで検出できた
